...
//*
//function ipLocation($datfile = "/usr/local/www/nginx/html/task/data/qqwry.dat"){
...
function getaddress($ip){
  ...
  * 获取ip地址信息
 */
function substr_address($ip){
	$address=$this->getaddress($ip);
	$address_str=iconv("gb2312","UTF-8",$address['area1']);
	$area=iconv("gb2312","UTF-8",$address['area2']);
	$provinces_array=array('安徽','北京','重庆','福建','甘肃','广东','广西','贵州','海南','河北','黑龙','河南','香港','湖北','湖南','江苏','江西','吉林','辽宁','澳门','内蒙','宁夏','青海','山东','上海','山西','陕西','四川','台湾','天津','新疆','西藏','云南','浙江','局域','本机','中国');
	$rel_address_array['nation']=$address_str;
	$address_array_str='';
	$address_array=explode('.', $address_str);
	$address_array_count=count($address_array);
	if($address_array_count>1){
		$provinces=substr($address_array[1], 0,6);
	}else{
		$provinces=substr($address_array[0], 0,6);
	}
	if(in_array($provinces,$provinces_array)){
		$rel_address_array['nation']='中国.'.$rel_address_array['nation'];
		$rel_address_array['nation_type']=0;
		switch ($provinces){
			case '黑龙':
				$rel_address_array['provinces']=$provinces.'江';	
				break;
			case '内蒙':
				$rel_address_array['provinces']=$provinces.'古';	
				break;
			case '局域':
				$rel_address_array['provinces']=$provinces.'网';	
				break;
			default:
				$rel_address_array['provinces']=$provinces;	
				break;
		}
	}else{
		$rel_address_array['nation_type']=1;
		$rel_address_array['provinces']=$address_str;
	}
	$rel_address_array['area']=$area;
	return $rel_address_array;
}
